...
from gridfm_datakit.network import Network, load_net_from_pglib, get_pglib_file_path
...
def pp_ieee14():
    ...
    return powsybl.pypowsybl.network.create_ieee14()
...
def load_net_ieee14():
    ...
    network_path = get_pglib_file_path("case14_ieee")
    return powsybl.load_net(network_path)
...
def pp_ieee9():
    ...
    return powsybl.pypowsybl.network.create_ieee9()
...
class TestFromPowsybl:
    ...
    def test_from_powsybl_returns_network(self, pp_ieee14):
        ...
        net = powsybl.from_powsybl(pp_ieee14)
    ...
    def test_from_powsybl_bus_matrix_shape(self, pp_ieee14):
        ...
        assert net.buses.shape[0] == len(buses_df), f"Should have {len(buses_df)} buses"
    ...
    def test_from_powsybl_custom_cost_coeffs(self, pp_ieee14):
        ...
        options = powsybl.ConversionOptions(gen_costs=gen_costs)
        net = powsybl.from_powsybl(pp_ieee14, options=options)
    ...
    def test_from_powsybl_ieee9(self, pp_ieee9):
        ...
        net = powsybl.from_powsybl(pp_ieee9)
    ...
    def test_from_powsybl_empty_network_raises(self):
        ...
        empty_net = powsybl.pypowsybl.network.create_empty("empty")

        with pytest.raises(ValueError, match="no buses"):
            powsybl.from_powsybl(empty_net)
...
class TestToPowsybl:
    ...
    def test_to_powsybl_returns_network(self, gridfm_case14):
        ...
        result = powsybl.to_powsybl(gridfm_case14)
        assert isinstance(result, powsybl.ConvertedNetwork), (
            "Should return a ConvertedNetwork object"
        )
        assert isinstance(result.pp_net, powsybl.pypowsybl.network.Network), (
    ...
    def test_to_powsybl_network_id(self, gridfm_case14):
        ...
        pp_net = powsybl.to_powsybl(gridfm_case14, network_id="test_network").pp_net
    ...
    def test_to_powsybl_bus_count(self, gridfm_case14):
        ...
        pp_net = powsybl.to_powsybl(gridfm_case14).pp_net
    ...
    def test_to_powsybl_loads_created(self, gridfm_case14):
        ...
        assert len(loads_df) == buses_with_load, f"Should have {buses_with_load} loads"

    def test_to_powsybl_case24(self, gridfm_case24):
        """Test conversion with case24_ieee_rts network."""

        pp_net = powsybl.to_powsybl(gridfm_case24).pp_net

        assert isinstance(pp_net, powsybl.pypowsybl.network.Network)
...
class TestRoundTrip:
    ...
    def test_roundtrip_bus_count(self, gridfm_case14):
        ...
        pp_net = powsybl.to_powsybl(gridfm_case14).pp_net
        net_back = powsybl.from_powsybl(pp_net)
    ...
    def test_roundtrip_powsybl_ieee14(self, pp_ieee14):
        ...
        gfm_net = powsybl.from_powsybl(pp_ieee14)
        pp_back = powsybl.to_powsybl(gfm_net).pp_net
...
class TestRoundTripWithLoadNet:
    ...
    def _assert_same_elements_same_values(self, original, roundtrip, label):
        ...
        np.testing.assert_allclose(
            self._numeric(original),
            self._numeric(aligned),
            rtol=1e-5,
    ...
    def test_roundtrip_with_load_net_ieee14_gens(self, load_net_ieee14):
        ...
        pp_net_roundtrip = powsybl.to_powsybl(powsybl.from_powsybl(pp_net)).pp_net

        self._assert_same_elements_same_values(
            pp_net.get_generators(),
            pp_net_roundtrip.get_generators(),
            "generators",
        )
    ...
    def test_generator_power_limits(self, pp_ieee14):
        ...
        for i, (gen_id, gen) in enumerate(gens_df.iterrows()):
            ...
            if not np.isnan(max_p):
                assert net.gens[i, PMAX] == max_p, f"PMAX mismatch for generator {i}"
            if not np.isnan(min_p):
                assert net.gens[i, PMIN] == min_p, f"PMIN mismatch for generator {i}"
...
class TestToPowsyblElementConservation:
    ...
    def test_conservation_case24(self, gridfm_case24):
        ...
        total_branches = len(pp_net.get_lines()) + len(
            pp_net.get_2_windings_transformers(),
        )
