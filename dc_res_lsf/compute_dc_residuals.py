...
def branch_rxb(fwd: np.ndarray, rev: np.ndarray) -> tuple:
    ...
    Inverts branch_vectors (gridfm_datakit/network.py:755): with tap = |tap| * exp(j*shift),
    ...
    # Out-of-service branches are stored with zero admittance, which would make the
    # inversion below divide by zero and yield NaN r/x/b.  NaN in the branch matrix
    # makes the Julia solver reject the whole case ("cannot convert float NaN to
    # integer"), so substitute a placeholder unit reactance for those rows.  They
    # carry BR_STATUS = 0, so the solver ignores their impedance entirely.
    dead = Ytf == 0
    ...
    r, x = z.real.copy(), z.imag.copy()
    shift = np.where(dead, 0.0, shift)
    r[dead] = 0.0
    x[dead] = 1.0  # any finite non-zero value; branch is out of service
    b = np.where(dead, 0.0, b)
    return r, x, b, tap, np.degrees(shift)
...
def assert_symmetric(fwd: np.ndarray, rev: np.ndarray) -> None:
    ...
    catch real asymmetry.  Out-of-service branches are skipped: their admittances are
    stored as exactly 0, so the relative comparison would be 0/0.
    """
    on = fwd[:, B_ON_E] == 1
    if not on.any():
        return
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Randomly sample N scenarios instead of taking the first N (0 = off). "
        "Prefer this over --limit on large datasets: .pt files are written in "
        "generation order, so the first N are not a representative sample.",
    )
    parser.add_argument(
        "--seed",
        type=int,
        default=0,
        help="Seed for --sample, so a run is reproducible.",
    )
    ...
    if args.sample:
        ...
        indices = sorted(
            rng.choice(
                indices,
                size=min(args.sample, len(indices)),
                replace=False,
            ).tolist(),
        )
    elif args.limit:
    ...
    print(
        f"Found {n_available} scenario files in {args.processed_dir}; "
        f"processing {len(indices)}",
    )

    # Timed separately from the solve loop: Julia/Ipopt startup and JIT warmup is a
    # large fixed cost, so folding it into the per-scenario average would badly
    # mislead anyone extrapolating to a full dataset.
    ...
    lines = [
        ...
        "forced to 0)",
        "",
        f"Julia startup : {startup_s:.1f} s (one-off: init_julia + Ipopt/JIT warmup)",
        f"Solve loop    : {loop_s:.1f} s for {len(df_out)} scenarios "
        f"= {1000 * loop_s / max(len(df_out), 1):.0f} ms/scenario",
        "",
    ]
