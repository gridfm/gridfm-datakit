...
- **Example Use Case**: Generating data to train a neural optimal power flow solver, or a power flow solver for secure cases
- **Performance**: Slower due to OPF solving for each scenario

### Unsecure Mode (`mode: "unsecure"`)
...
- **Constraint Validation**: Generator limits, branch loading (secure mode only)
