    ...
    **Notes**
    ...
    def __call__(
        ...
        with open(scenarios_log, "a") as f:
            f.write("u=" + str(u) + "\n")
            f.write("l=" + str(lower) + "\n")
            f.write("using power factor between 0.8 and 1\n")
        ...
        if self.change_reactive_power:
            # pf sampled between 0.8 and 1
            power_factor = np.random.uniform(0.8, 1, size=load_profile_pmw.shape)
            tan_arccos_pf = np.tan(np.arccos(power_factor))
            load_profile_qmvar = load_profile_pmw * tan_arccos_pf
            noise = np.random.uniform(
                1 - self.sigma,
                1 + self.sigma,
